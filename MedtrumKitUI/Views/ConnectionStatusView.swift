...
final class ConnectionStatusViewModel: ObservableObject, PumpManagerStatusObserver {
    ...
    private func updateState(_ pumpManager: MedtrumPumpManager) {
        // Both flags count as connecting: the first spans the auth flow, the second the retry gaps.
        if pumpManager.state.isConnected {
            status = .connected
        } else if pumpManager.state.isConnecting || pumpManager.state.isSearchingForBase {
            status = .connecting
        } else {
            status = .disconnected
        }
...
struct ConnectionStatusIcon: View {
    ...
    var body: some View {
        Image(systemName: symbolName)
            ...
            .connectionStatusBreathe(isActive: status == .connecting)
    }
...
private extension View {
    /// Breathes while connecting and settles the instant it lands. Breathe needs iOS 18, pulse 17.
